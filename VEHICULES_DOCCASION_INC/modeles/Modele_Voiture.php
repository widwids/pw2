<?php
	class Modele_Voiture extends TemplateDAO {
		...
		public function modifPhotoVoiture($nomPhoto, $ordre, $autoId) {
			
			$requete = "UPDATE photo SET nomPhoto = :nomPhoto, ordre = :ordre, autoId = :autoId 
						WHERE autoId = :autoId AND ordre = :ordre" ;
			$requetePreparee = $this -> connexion -> prepare($requete);
            $requetePreparee -> bindParam(":nomPhoto", $nomPhoto);
            $requetePreparee -> bindParam(":ordre", $ordre);
            $requetePreparee -> bindParam(":autoId", $autoId);
